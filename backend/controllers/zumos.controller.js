const zumosModel = require('../models/zumos.model');

exports.listJuices = async (req, res) => {
    try {
        const resultados = await zumosModel.getAllJuices();
        res.send(resultados)
    } catch (error) {
        res.send(error)
    }
...
exports.getSingleJuiceByID = async (req, res) => {
    try {
        // Sacar del path param el ID del producto
        const id = req.params.id;
        //Pedir al Modelo que saque los datos de los zumos por su ID
        const zumo = await zumosModel.getJuiceByID(id);
...
exports.createNewJuice = async (req, res) => {
    ...
    if (errors.isEmpty()) {
        ...
        try {
            const result = await zumosModel.insertSingleJuice(nombre, tipo, fk_productos)
...
exports.updateSingleJuice = async (req, res) => {
    ...
    if (errors.isEmpty()) {
        ...
        // Llamo al modelo:
        try {
            const result = await productosModel.updateProduct(id, nombre, tipo);
...
exports.deleteSingleJuice = async (req, res) => {
    //Coger de los path params el ID
    const id = req.params.id;
    //Pedir al modelo que elimine ese producto
    try {
        const results = await productosModel.removeJuice(id);
